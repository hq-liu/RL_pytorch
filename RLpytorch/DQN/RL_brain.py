...
class deep_q_network(nn.Module):
    def __init__(self, state_dim, action_dim, n_layers, hidden_size):
        ...
        self.hidden_layers = nn.Linear(in_features=self.hidden_size,
...
class deep_q_learning():
    def __init__(self, state_dim, action_dim, learning_rate=1e-3, reward_decay=0.9,
                 ...
                 e_greedy_increment=None, dtype=torch.cuda.FloatTensor):
        self.state_dim = state_dim
        self.action_dim = action_dim
        ...
        self.target_net = deep_q_network(self.state_dim, self.action_dim, 3, 128).type(dtype)
        print(self.target_net)
        # evaluation_net: input: s_prime; output: evaluation of q value
        self.eval_net = deep_q_network(self.state_dim, self.action_dim, 3, 128).type(self.dtype)
    ...
    def choose_action(self, observation):
        ...
        :return: 动作值
        """
        observation = Variable(torch.FloatTensor(observation)).type(self.dtype)
    ...
    def learn(self):
        ...
        state, action, reward, next_state = Variable(torch.FloatTensor(state)).type(self.dtype), \
                                            Variable(torch.LongTensor(action)).type(torch.cuda.LongTensor), \
                                            Variable(torch.FloatTensor(reward)).type(self.dtype), \
                                            Variable(torch.FloatTensor(next_state)).type(self.dtype)

        q_next = self.target_net(next_state)
        next_state_values = torch.max(q_next, dim=1)[0]
