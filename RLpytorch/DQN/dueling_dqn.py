...
class dueling_dqn(nn.Module):
    def __init__(self, state_dim, action_dim, n_layers, hidden_size):
        ...
        self.advantage = nn.Linear(in_features=self.hidden_size, out_features=action_dim)
...
class dueling_dqn_learn():
    def __init__(self, state_dim, action_dim, learning_rate=1e-3, reward_decay=0.9,
        ...
        self.target_net = dueling_dqn(self.state_dim, self.action_dim, 1, 32).type(self.FloatTensor)
        # evaluation_net: input: s_prime; output: evaluation of q value
        self.eval_net = dueling_dqn(self.state_dim, self.action_dim, 1, 32).type(self.FloatTensor)
    ...
    def store_params(self):
        torch.save(self.eval_net.state_dict(), 'dueling_dqn.pkl')

    def load_params(self):
        self.eval_net.load_state_dict(torch.load('dueling_dqn.pkl'))
